...
class CSVPlayer(QMainWindow):
    ...
    def play_selected_video(self):
        ...
        if current_row >= 0 and current_row < len(self.csv_data):
            ...
            if video_url:
                self.download_and_play_video(video_url, work_name)
            else:
                QMessageBox.warning(self, "警告", "该作品没有有效的视频链接")
                
    def download_and_play_video(self, url, work_name):
        """下载并播放视频"""
        ...
        self.status_bar.showMessage(f"正在下载视频: {work_name}")
        ...
        # 开始下载
        self.downloader = VideoDownloader(url, temp_file)
    ...
    def on_download_error(self, error_msg):
        """下载错误处理"""
        self.progress_bar.setVisible(False)
        QMessageBox.critical(self, "下载错误", f"视频下载失败:\n{error_msg}")
